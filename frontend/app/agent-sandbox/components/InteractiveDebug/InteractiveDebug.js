...
const InteractiveDebug = ({
      ...
      setMessages([
        {
          ...
          type: 'system',
          content:
            'Welcome to Interactive Agent Debugging! Ask me about agent states, decision logic, or explore the processing timeline. Try commands like "explain risk analysis" or "show memory retrieval".',
          timestamp: new Date(),
  ...
  const generateAgentResponse = (input) => {
    ...
    if (
      ...
      return {
        type: 'agent',
        content: `The Risk Analyzer is currently processing ${
          selectedScenario.name
        } with confidence ${(Math.random() * 0.2 + 0.8).toFixed(
          2
        )}. Key factors:\n\n• Transaction velocity: ${
          Math.random() > 0.5 ? 'HIGH' : 'NORMAL'
        }\n• Geographic anomaly: ${
          Math.random() > 0.7 ? 'DETECTED' : 'NONE'
        }\n• Historical patterns: ${Math.round(
          Math.random() * 20 + 80
        )}% match\n• Risk score: ${Math.round(
          metrics.accuracy + Math.random() * 10
        )}/100`,
        agent: 'Risk Analyzer',
        confidence: 0.94,
        debugData: {
    ...
    if (
      ...
      lowerInput.includes('retrieval')
    ) {
      return {
        type: 'agent',
        content: `Memory Agent has retrieved ${Math.round(
          Math.random() * 500 + 500
        )} similar cases from MongoDB Atlas vector search:\n\n• Vector similarity: 92.4%\n• Time range: Last 90 days\n• Pattern matches: ${Math.round(
          Math.random() * 15 + 10
        )}\n• Confidence decay: Applied (time-weighted)\n• Memory consolidation: ${
          Math.random() > 0.6 ? 'Active' : 'Inactive'
        }`,
        agent: 'Memory Agent',
        confidence: 0.89,
        debugData: {
    ...
    if (
      ...
      return {
        ...
        content: `Decision Engine final output: **${decision}**\n\nDecision factors:\n• Risk threshold: ${
          decision === 'BLOCK' ? 'EXCEEDED' : 'WITHIN LIMITS'
        }\n• Consensus score: ${(Math.random() * 0.15 + 0.85).toFixed(
          2
    ...
    if (
      ...
      return {
        type: 'agent',
        content: `Current processing timeline for Case #${Math.random()
          .toString(36)
          .substr(2, 9)
    ...
    if (
      ...
      return {
        ...
        content: `Current Performance Metrics:\n\n• Accuracy: ${
          ...
          metrics.fraudPrevented / 1000000
        ).toFixed(1)}M`,
        ...
        >
          ...
          >
            ...
            <div>
              ...
              >
                Conversational Debugging • Time-Travel Analysis •
          ...
          >
            <Badge
              variant={isSimulationRunning ? 'green' : 'lightgray'}
            >
              {isSimulationRunning ? 'LIVE DEBUG' : 'STATIC MODE'}
            </Badge>
            <Badge variant="blue">{selectedScenario.name}</Badge>
          </div>
        </div>
      </div>
      ...
      >
        ...
        >
          ...
          >
            ...
            >
              Ask questions about agent behavior, decision logic, or
              processing flow
            </Body>
          </div>
