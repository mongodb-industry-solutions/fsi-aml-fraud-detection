...
class NetworkRepository(NetworkRepositoryInterface):
        ...
        try:
            ...
            for relationship in network_data["relationships"]:
                ...
                if isinstance(strength_value, (int, float)):
                    ...
                    strength_enum = RelationshipStrength(strength_value) if strength_value in RelationshipStrength._value2member_map_ else RelationshipStrength.POSSIBLE
            ...
            if entity_ids:
                ...
                for entity in entities:
                    entity_id = entity.get("entityId", str(entity["_id"]))
                    ...
                    # Handle entity name - could be string or object
                    ...
                    node = NetworkNode(
                        ...
                        risk_level=NetworkRiskLevel(entity.get("riskAssessment", {}).get("overall", {}).get("level", "low")),
    ...
    async def get_entity_connections(self, entity_id: str,
        ...
        try:
            # Build match conditions using new schema
            match_conditions = {
                "$or": [
            ...
            pipeline = (self.aggregation()
                       ...
                       .lookup(
                           ...
                           local_field="source.entityId",
                           foreign_field="entityId",
                           as_field="source_entity"
                       )
                       .lookup(
                           from_collection=self.entity_collection_name,
                           local_field="target.entityId", 
                           foreign_field="entityId",
            ...
            for result in results:
                ...
                if result["source"]["entityId"] == entity_id:
                ...
                if connected_entity:
                    connection = {
                        ...
                        "connected_entity_id": connected_entity.get("entityId", connected_entity.get("_id", "unknown")),
                        "connected_entity_name": connected_entity.get("name", "Unknown"),
                        "entity_type": connected_entity.get("entityType", "unknown"),
                        "relationship_type": result.get("type"),
                        "confidence_score": result.get("confidence", 0.0),
                        "strength": result.get("strength", "possible"),
                        "verified": result.get("verified", False),
                        "direction": direction,
                        "risk_level": connected_entity.get("riskAssessment", {}).get("overall", {}).get("level", "low")
                    }
    ...
    async def find_relationship_path(self, source_entity_id: str,
                                   ...
                                   max_depth: int = 6,
                                   relationship_types: Optional[List[RelationshipType]] = None,
                                   only_active: bool = True) -> Optional[List[Dict[str, Any]]]:
        """Find shortest path between two entities using BFS with new schema"""
        try:
            if source_entity_id == target_entity_id:
                return []
            
            logger.info(f"Finding path from {source_entity_id} to {target_entity_id} (max depth: {max_depth})")
            
            # BFS implementation for pathfinding
            queue = deque([(source_entity_id, [])]) # (current_entity, path_so_far)
            ...
            while queue and depth < max_depth:
                ...
                for _ in range(level_size):
                    current_entity_id, path_so_far = queue.popleft()
                    
                    # Get connections from current entity using new schema
                    ...
                    for connection in connections:
                        ...
                        # Skip if only_active and relationship is inactive
                        if only_active and not connection.get("verified", True):
                            continue
                        
                        # Build relationship step
                        relationship_step = {
                            "relationship_id": connection["relationship_id"],
                        ...
                        new_path = path_so_far + [relationship_step]
                        
                        # Check if we've reached the target
                        if connected_id == target_entity_id:
                            logger.info(f"Path found with {len(new_path)} steps (depth: {depth})")
                            return new_path
                ...
                logger.debug(f"Depth {depth}: explored {level_size} entities, {len(queue)} in queue")
            
            logger.info(f"No path found between {source_entity_id} and {target_entity_id}")
            return None
    ...
    async def _get_entities_batch(self, entity_ids: List[str]) -> List[Dict[str, Any]]:
        ...
        try:
            # Entity IDs in relationships collection are strings like "CDI0A-560467CDF1"
            # We need to search by entityId field, not _id
