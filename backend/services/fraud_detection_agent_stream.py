...
class FraudDetectionAgentStream:
    ...
    async def evaluate_transaction(self, transaction: Dict) -> Dict:
        """
        Evaluate a transaction for fraud using the AI agent with streaming support.
        This implementation uses Microsoft's recommended streaming approach instead of polling.
        """
        ...
        try:
            ...
            max_stream_events = 1000  # Safety limit
            
            print(f"🚀 Starting streaming evaluation for transaction {transaction.get('transaction_id', 'unknown')}")
            
            # Use streaming instead of polling - Microsoft's recommended approach
            with self.agents_client.runs.stream(
                ...
                for event_type, event_data, _ in stream:
                    ...
                    if stream_events_processed > max_stream_events:
                        print(f"⚠️ Maximum stream events ({max_stream_events}) reached. Stopping stream.")
                        break
                    
                    # Handle different event types
                    if event_type == AgentStreamEvent.THREAD_RUN_CREATED:
                        print(f"📋 Thread run created: {event_data.id}")
                    
                    elif event_type == AgentStreamEvent.THREAD_RUN_IN_PROGRESS:
                        print(f"⏳ Processing transaction analysis...")
                    
                    elif event_type == AgentStreamEvent.THREAD_RUN_REQUIRES_ACTION:
                        # Handle tool calls in real-time
                        if hasattr(event_data, 'required_action') and event_data.required_action:
                            tool_outputs = []
                            tool_calls = event_data.required_action.submit_tool_outputs.tool_calls
                            tool_call_count += len(tool_calls)
                            
                            print(f"🔧 Processing {len(tool_calls)} tool calls (Total: {tool_call_count})...")
                        ...
                        # Collect the agent's response in real-time
                        if isinstance(event_data, MessageDeltaChunk):
                            if event_data.delta and event_data.delta.content:
                                for content in event_data.delta.content:
                                    if hasattr(content, 'text') and content.text:
                                        if hasattr(content.text, 'value'):
                                            agent_response += content.text.value
                                            # Optional: Show real-time response building
                                            # print(content.text.value, end='', flush=True)
                    
                    elif event_type == AgentStreamEvent.THREAD_MESSAGE_COMPLETED:
                        print(f"💬 Agent message completed")
                        ...
                        if hasattr(event_data, 'last_error'):
                            print(f"   Error details: {event_data.last_error}")
                        break
                    
                    elif event_type == AgentStreamEvent.THREAD_RUN_CANCELLED:
                        print(f"🚫 Run cancelled")
                        break
                    
                    elif event_type == AgentStreamEvent.THREAD_RUN_EXPIRED:
                        print(f"⏰ Run expired")
                        break
            ...
            print(f"\n📊 Stream statistics: {stream_events_processed} events processed, {tool_call_count} tool calls executed")
            
            # If we didn't get a response through streaming, fallback to checking messages
            if not agent_response:
                print("📨 Retrieving final message from thread...")
            ...
            if not agent_response:
                return {
                    "decision": "ESCALATE",
                    "risk_score": 75,
                    "reasoning": "Agent failed to provide response through streaming",
                    "streaming_stats": {
                        "events_processed": stream_events_processed,
            ...
            return {
                "decision": "ESCALATE",
                "risk_score": 80,
                "reasoning": f"Error during streaming agent analysis: {str(e)}",
                "streaming_stats": {
                    "events_processed": stream_events_processed if 'stream_events_processed' in locals() else 0,
                    "tool_calls": tool_call_count if 'tool_call_count' in locals() else 0
                },
                "agent_analysis": False,
