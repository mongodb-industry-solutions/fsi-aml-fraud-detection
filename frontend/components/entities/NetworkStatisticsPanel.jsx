...
function NetworkStatisticsPanel({ 
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: spacing[4] }}>
      ...
      {/* Key Entities */}
      {(hubEntities.length > 0 || prominentEntities.length > 0) && (
        <Card style={{ padding: spacing[4] }}>
          <H3 style={{ 
            marginBottom: spacing[3],
            display: 'flex',
            ...
            <Icon glyph="Star" style={{ color: palette.yellow.base }} />
            Key Entities
          </H3>
          ...
          <div style={{ 
            ...
            {/* Hub Entities */}
            <div>
              <H3 style={{ marginBottom: spacing[2] }}>Hub Entities (Most Connected)</H3>
              {hubEntities.length > 0 ? (
                <div style={{ display: 'flex', flexDirection: 'column', gap: spacing[2] }}>
                  {hubEntities.slice(0, 5).map((entity, index) => (
                    <div key={entity.entityId || index} style={{ 
                      padding: spacing[2],
                      border: `1px solid ${palette.gray.light1}`,
                      borderRadius: '4px',
                      backgroundColor: entity.entityId === centerEntityId ? palette.blue.light3 : 'transparent'
                    }}>
                      <Body weight="medium" style={{ 
                        fontFamily: 'monospace',
                        color: entity.entityId === centerEntityId ? palette.blue.dark2 : palette.gray.dark2
                      }}>
                ...
                <Body style={{ color: palette.gray.base }}>No hub entities identified</Body>
              )}
            </div>

            {/* Prominent Entities */}
            <div>
              <H3 style={{ marginBottom: spacing[2] }}>Prominent Entities (High Influence)</H3>
              {prominentEntities.length > 0 ? (
                <div style={{ display: 'flex', flexDirection: 'column', gap: spacing[2] }}>
                  {prominentEntities.slice(0, 5).map((entity, index) => (
                    <div key={entity.entityId || index} style={{ 
                      padding: spacing[2],
                      border: `1px solid ${palette.gray.light1}`,
                      borderRadius: '4px',
                      ...
                      <Body style={{ fontSize: '12px', color: palette.gray.base }}>
                        Prominence: {entity.prominence_score?.toFixed(1)} | Risk: {entity.risk_score}%
                      </Body>
                    </div>
                  ))}
                </div>
              ) : (
                <Body style={{ color: palette.gray.base }}>No prominent entities identified</Body>
              )}
            </div>
          </div>
        </Card>
